...
>**Ghi chú:**
>
>Người ta có thể nhầm lẫn các element với một khái niệm được biết rộng hơn về các "component". Chúng tôi sẽ giới thiệu các "component" trong [phần tới](./components-and-props.md). Các element là những gì mà các component được "làm từ", và chúng tôi khuyến khích bạn đọc phần này trước khi đi đến phần kế tiếp.
...
Với sự hiểu biết của chúng tôi cho đến thời điểm này, thì chỉ có một cách duy nhất để cập nhật "giao diện" (UI) đó là tạo ra một element mới và truyền nó vào [`ReactDOM.render()`](./react-dom.md#render).
...
Nó gọi [`ReactDOM.render()`](./react-dom.md#render) mỗi giây từ một "lời gọi lại" (callback) [`setInterval()`](https://developer.mozilla.org/en-US/docs/Web/API/WindowTimers/setInterval).

>**Ghi chú:**
>
>Trong thực tế, hầu hết các ứng dụng React chỉ gọi [`ReactDOM.render()`](./react-dom.md#render) một lần duy nhất. Trong những phần tiếp theo, chúng ta sẽ tìm hiểu làm thế nào để "mã" (code) được đóng gói vào ["những component có trạng thái" (stateful components)](./state-and-lifecycle.md).
>
...
`Thanh kiểm tra` (inspector) DOM chỉ ra những cập nhật cần thiết

!["thanh kiểm tra" (inspector) DOM chỉ ra những cập nhật cần thiết](./images/granular-dom-updates.gif)
