...
Xem xem lại ví dụ đồng hồ tíc tắc [một phần của chương trước](./rendering-elements.md#updating-the-rendered-element). Trong [Rendering Elements](./rendering-elements.md#rendering-an-element-into-the-dom), chúng ta chỉ học cách để update UI. chúng ta gọi `ReactDOM.render()` để thay đổi đầu ra được kết xuất (rendered output):
